import { uuid, Dialogs } from "@jsplumbtoolkit/browser-ui"

import {ONE_DAY_IN_MILLISECONDS, STEP_WIDTH, TYPE_TASK_GROUP} from "./constants"
...
function _addNew(gantt:Gantt, type:string, dialogId:string) {
...
export function addNewTask(gantt:Gantt) {
    _addNew(gantt, TYPE_TASK, DIALOG_NEW_TASK)
}

export function addNewTaskGroup(gantt:Gantt) {
    _addNew(gantt, TYPE_TASK_GROUP, DIALOG_NEW_TASK_GROUP)
}

export function addNewMilestone(gantt:Gantt) {
    _addNew(gantt, TYPE_MILESTONE, DIALOG_NEW_MILESTONE)
}
